...
export default function EditDataForm(props) {
  ...
  return (
    <>
      <Modal opened={props.isOpen} withCloseButton onClose={handleCloseModal} size="md" radius="md" title="Edit Food">
        <form onSubmit={form.onSubmit(() => mutate())}>
          <TextInput withAsterisk label="Name" placeholder="Input your name food" {...form.getInputProps("name")} />
          <Select
            label="Category"
            withAsterisk
          ...
          />
          <Textarea style={{ marginTop: "10px" }} withAsterisk label="Description" placeholder="Input your description product" {...form.getInputProps("description")} />

          <TextInput withAsterisk label="CreatedBy" placeholder="Input your createdBy" {...form.getInputProps("createdBy")} />
